...
//      float llum = getSensors().getLightSensorLeft().getAverageLuminance();
  //    float rlum = getSensors().getLightSensorRight().getAverageLuminance();
      ...
      double llum = getSensors().getLightSensorLeft().getLux();
      double  rlum = getSensors().getLightSensorRight().getLux();
      
      llum= (float) Math.pow(llum,0.1);
      rlum =(float) Math.pow(rlum,0.1);
      
      double translationalVelocity = 0.5 / (llum + rlum);
      double rotationalVelocity = (llum - rlum) * Math.PI / 2;
